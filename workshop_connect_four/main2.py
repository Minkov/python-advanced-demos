...
def apply_player_choice(board, player_choice, player):
    board[player_choice].append(player)
    return player_choice, len(board[player_choice]) - 1
...
def is_player_choice_valid(board, player_choice, rows_count):
    is_column_index_valid = 0 <= player_choice < rows_count
    return is_column_index_valid
...
def create_board(rows_count, columns_count):
    ...
    return [[] * rows_count for _ in range(columns_count)]
